...
if [ $? -eq 10 ]; then
    save_launcher "$1"
    echo -e "{ \"type\":\"info\", \"size\":2, \"duration\":3000, \"x\":400, \"y\":0,  \"message\":\"Game preset created: $Launcher_name\",  \"icon\":\"\" }" >/tmp/trimui_osd/osd_toast_msg
fi
...
if [ $? -eq 10 ]; then
    save_launcher
    echo -e "{ \"type\":\"info\", \"size\":2, \"duration\":3000, \"x\":400, \"y\":0,  \"message\":\"Emu preset created: $Launcher_name\",  \"icon\":\"\" }" >/tmp/trimui_osd/osd_toast_msg
fi
